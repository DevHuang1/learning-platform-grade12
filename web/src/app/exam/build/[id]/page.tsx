...
export default function ExamBuilderPage() {
  ...
  const [confirmRemoval, setConfirmRemoval] = useState<
    { kind: "section"; id: number } | { kind: "question"; id: number } | null
  >(null);
  ...
  const [newQuestions, setNewQuestions] = useState<
    Record<number, QuestionDraft>
  >({});
  const [addingQuestionFor, setAddingQuestionFor] = useState<number | null>(
    null,
  );
  ...
  async function publishSheet() {
    ...
    try {
      ...
      const message =
        e instanceof Error ? e.message : "Failed to publish sheet";
      setError(message);
  ...
  async function handleAddQuestion(sectionId: number) {
    ...
    try {
      ...
      await insertQuestion(sectionId, {
        ...
        correct_option:
          d.question_type === "multiple_choice" ? d.correct_option : 0,
        image_path,
        image_url,
      });
      const updated = await fetchExamSheet(sheet.id);
      ...
      setNewQuestions((m) => ({
        ...m,
        [sectionId]: { ...DEFAULT_QUESTION_DRAFT },
      }));
    ...
    try {
      await supabase
        ...
        .update({
          ...
          ...(patch.answer_guide !== undefined
            ? { answer_guide: patch.answer_guide }
            : {}),
          ...(patch.marks !== undefined
            ? { marks: Math.max(1, Number(patch.marks) || 1) }
            : {}),
          ...(patch.question_type !== undefined
            ? { question_type: patch.question_type }
            : {}),
          ...(patch.options !== undefined ? { options: patch.options } : {}),
          ...(patch.correct_option !== undefined
                ...
                questions: s.questions.map((q) =>
                  q.id === id
                    ? {
                        ...q,
                        prompt:
                          patch.prompt !== undefined ? patch.prompt : q.prompt,
                        answer_guide:
                          patch.answer_guide !== undefined
                            ? patch.answer_guide
                            : q.answer_guide,
                        marks:
                          patch.marks !== undefined
                            ? Math.max(1, Number(patch.marks) || 1)
                            : q.marks,
                        question_type:
                          patch.question_type !== undefined
                            ? patch.question_type
  ...
  async function handleUploadImage(
    q: ExamWithSections["sections"][number]["questions"][number],
    file: File,
  ) {
    try {
      const res = await uploadImage(
      ...
      setSheet((prev) =>
        prev
          ? {
              ...
              sections: prev.sections.map((s) =>
                s.id === section.id
                  ? { ...s, image_path: null, image_url: null }
                  : s,
              ),
            }
          : prev,
        ...
        prev
          ? {
              ...
              sections: prev.sections.map((s) => ({
                ...
                questions: s.questions.map((x) =>
                  x.id === q.id
                    ? { ...x, image_path: null, image_url: null }
                    : x,
                ),
              })),
            }
        ...
        {!hasSupabase() && (
          <Card className="mb-4 border-amber-200 bg-amber-50">
            ...
            <p className="mt-1 text-sm text-amber-700">
              You can&apos;t build or edit exam sheets without a Supabase
              backend.
            </p>
          </Card>
        )}
  ...
  return (
    <Shell>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatBox
          label="Questions"
          value={totals.question_count}
          icon={<QuestionIcon />}
        />
        <StatBox
          label="Total Marks"
          value={totals.total_marks}
          accent="text-indigo-600"
          icon={<MarksIcon />}
        />
        <StatBox
          label="Duration"
          value={`${sheet.duration_minutes}m`}
          icon={<ClockIcon />}
        />
        <StatBox
          label="Status"
          value={
            <Badge tone={STATUS_TONE[sheet.status] || "gray"}>{status}</Badge>
          }
          icon={<StatusIcon />}
        />
      </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {hasSupabase() && (
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setConfirmRemoval({ kind: "section", id: section.id })
                  }
                >
                  <TrashIcon />
                  Delete
              ...
              <div className="mt-2 overflow-hidden border border-gray-200 bg-white">
                ...
                <img
                  src={
                    section.image_url ||
                    (section.image_path
                      ? getPublicUrl(QUESTION_IMAGES_BUCKET, section.image_path)
                      : undefined)
                  }
                  alt="Section image"
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {hasSupabase() && (
                          <Button
                            ...
                            className="px-2 py-1 text-xs"
                            onClick={() =>
                              setConfirmRemoval({ kind: "question", id: q.id })
                            }
                          >
                            <TrashIcon />
                            Remove
                      ...
                      <textarea
                        ...
                        onBlur={() =>
                          persistQuestion(q.id, { prompt: d.prompt })
                        }
                        rows={2}
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        <input
                          ...
                          value={d.marks}
                          onChange={(e) =>
                            patchQuestion(q.id, { marks: e.target.value })
                          }
                          onBlur={() =>
                            persistQuestion(q.id, { marks: d.marks })
                          }
            ...
            {addingQuestionFor === section.id ? (
              <div className="mt-4 border border-dashed border-indigo-300 bg-indigo-50 p-3">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[11px] uppercase tracking-[0.08em] text-indigo-700">
                    New question
                  </span>
                  <Button
                    variant="ghost"
                          ...
                          <Button
                            ...
                            onClick={() =>
                              setNewQuestions((m) => {
                                ...
                                const correct_option =
                                  prev.correct_option >= options.length
                                    ? options.length - 1
                                    : prev.correct_option;
                                return {
                                  ...m,
                ...
                <div className="mt-3">
                  ...
                  {newQuestionImage ? (
                    <div className="relative overflow-hidden border border-gray-200 bg-white">
                      ...
                      <div className="flex items-center justify-between border-t border-gray-200 px-3 py-1.5">
                        ...
                        <Button
                          ...
                          className="px-2 py-1 text-xs"
                          onClick={() => {
                            URL.revokeObjectURL(newQuestionImage.preview);
                            setNewQuestionImage(null);
                          }}
                        >
                          Remove
                    ...
                    <label className="flex cursor-pointer items-center justify-center gap-2 border border-dashed border-gray-300 bg-white px-3 py-2 font-mono text-[11px] uppercase tracking-[0.08em] text-gray-500 transition-colors hover:border-indigo-400 hover:text-indigo-600">
                      <input
                        type="file"
                        accept="image/*"
                        className="hidden"
                ...
                <Button
                  className="mt-3"
                  disabled={!nd.prompt.trim() || uploadingNewImage}
                  onClick={() => handleAddQuestion(section.id)}
                >
                  <PlusIcon />
                  {uploadingNewImage ? "Uploading…" : "Add question"}
                </Button>
              </div>
            ) : (
              hasSupabase() && (
      ...
      {addingSection ? (
        <Card className="border-dashed border-indigo-300 bg-indigo-50">
              ...
              <div className="relative overflow-hidden border border-gray-200 bg-white">
                ...
                <div className="flex items-center justify-between border-t border-gray-200 px-3 py-1.5">
                  ...
                  <Button
                    ...
                    className="px-2 py-1 text-xs"
                    onClick={() => {
                      URL.revokeObjectURL(newSectionImage.preview);
                      setNewSectionImage(null);
                    }}
                  >
                    Remove
          ...
          <Button
            className="mt-3"
            disabled={!newSectionTitle.trim() || uploadingNewImage}
            onClick={handleAddSection}
          >
            <PlusIcon />
            {uploadingNewImage ? "Uploading…" : "Add section"}
          </Button>
        </Card>
      ) : (
        hasSupabase() && (
